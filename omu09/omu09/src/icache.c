...
# else
/*			l3tol(iptr->i_addr, diptr->di_addr, 13); */
			for (i = 0; i < 13; i++)
				iptr->i_addr[i] = diptr->di_addr[i].lo_w;
# endif
...
struct dev *mdev;
{
	...
	b_ptr = getbuf(mdev, dminor, ((ino-1) / INOPB) + 2);
	return b_ptr;
}
...
# else
		for (i = 0; i < 13; i++) {
			diptr->di_addr[i].lo_w  = iptr->i_addr[i];
			diptr->di_addr[i].hi_b  = 0;
# endif
		}
